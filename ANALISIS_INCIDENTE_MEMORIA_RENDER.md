...
**Nota de adaptación (portado a `origin/main` post-Mission #002):** este
documento se escribió contra el `analizar_plano()` síncrono de antes de
Mission #002 (Plan Processing Stability), que Mission #002 reemplazó por
`iniciar_analisis_plano()` + `_completar_analisis_plano()` +
`analizar_plano_sincrono()` (ver `api/repositorio_proyectos.py`). Las
mediciones de memoria y la conclusión de causa raíz de abajo siguen
siendo válidas tal cual -- son sobre el proceso hijo que hace el trabajo
pesado (`_procesar_plano_pdf`, sin cambios de Mission #002), no sobre
cómo el proceso principal orquesta o espera ese resultado. El logging
estructurado de la sección 4.3 sí se adaptó a las funciones actuales al
portar este trabajo -- ver el log real en
`_completar_analisis_plano()`, no en `analizar_plano()` (que ya no
existe con ese nombre). Cualquier referencia a `analizar_plano()` más
abajo es del contexto histórico en el que se escribió esta
investigación.
