...
2. **`app/producto/[id]/page.tsx` no tiene forma de cargar un producto por id vía red** -- solo lee de `sessionStorage`, poblado al navegar desde resultados de búsqueda. Cualquier link directo, compartido o recargado (F5) a una página de producto muestra "Producto no disponible", indistinguible de un producto realmente eliminado. Encontrado en la auditoría de UX, **no lo arreglé** porque requiere un endpoint nuevo (`GET /productos/{proveedor}/{id}`) -- exactamente el tipo de "función nueva" que esta misión pidió no agregar. Es el hueco de mayor riesgo real de esta lista si algún cliente comparte o guarda en favoritos un link de producto.
...
- **El endpoint `GET /productos/{id}` para producto/[id]/page.tsx** (riesgo #2 de arriba) -- es la próxima función real que valdría la pena, pero es una función nueva, no un endurecimiento, y esta misión pidió explícitamente no agregar ninguna.
...
494 pruebas de backend (unittest, sin las de crawlers que dependen de red), suite completa, cero regresiones -- incluye una prueba nueva que fija el resultado exacto de `listar_proyectos()` antes y después del cambio de SQL. `npx tsc --noEmit` limpio. ESLint: de 4 errores / 11 warnings iniciales a 2 errores / 9 warnings, los que quedan documentados arriba como deliberadamente diferidos. Verificación visual con Playwright contra los dos servidores reales corriendo: registro, simulación de caída de red en la lista de proyectos (mensaje claro + "Reintentar" recupera sin recargar), proyecto inexistente sigue mostrando "no encontrado" en vez de "error de red", flujo normal de crear proyecto intacto.
