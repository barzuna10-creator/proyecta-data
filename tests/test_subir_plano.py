...
mensaje 413 claro y el log estructurado del rechazo, contra el
comportamiento de TAMANO_MAXIMO_PLANO_BYTES tal cual sea (no es un
mecanismo de memoria -- ver el comentario junto a la constante -- estas
pruebas parchan su valor a algo chico solo para no necesitar un archivo
real de decenas de MB en la suite). No corre lectura_planos de verdad -- se mockea
repo.analizar_plano() (esa función ya tiene su propia cobertura en
test_analizar_plano.py), así que esto prueba solo la responsabilidad del
router: medir el archivo mientras lo escribe a disco, cortar si excede
el límite, y pasarle el tamaño real a analizar_plano() cuando no lo
excede.
...
class PruebaLimiteDeTamano(BasePruebaIntegracion):
    ...
    def test_archivo_que_supera_el_limite_da_413_con_el_limite_en_el_mensaje(self):
        ...
        with mock.patch.object(repo, "analizar_plano") as analizar_falso:
    ...
    def test_archivo_que_supera_el_limite_queda_registrado_en_el_log(self):
        ...
        with mock.patch.object(repo, "analizar_plano"):
    ...
    def test_archivo_dentro_del_limite_llega_a_analizar_plano_con_su_tamano_real(self):
