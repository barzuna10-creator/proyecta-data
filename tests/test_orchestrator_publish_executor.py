...
class CheckBeforeCreateTests(PublishExecutorTestCase):
    def test_creates_pr_when_none_exists(self):
        ...
        calls = [
            _ok_result(),  # git push
            _json_result([]),  # gh pr list -> none found
            _json_result({"number": 7, "url": "https://example.invalid/pr/7"}),  # gh pr create
        ...
        with mock.patch.object(publish_executor, "_run", side_effect=calls):
    ...
    def test_reuses_existing_open_pr_never_creates_a_second_one(self):
        ...
        with mock.patch.object(publish_executor, "_run", side_effect=calls) as run_mock:
            result = publish_executor.run(
                mid, repository_root="/tmp/repo", branch="b", pr_title="t",
                ci_poll_timeout_seconds=5, ci_poll_interval_seconds=0.01,
            )
    ...
    def test_pending_forever_reaches_bounded_timeout_and_blocks(self):
        ...
        def side_effect(*args, **kwargs):
            ...
            if "list" in argv:
                return _json_result([])
            if "create" in argv:
                return _json_result({"number": 1, "url": "https://example.invalid/pr/1"})
            return pending_view

        with mock.patch.object(publish_executor, "_run", side_effect=side_effect):
            result = publish_executor.run(
                mid, repository_root="/tmp/repo", branch="b", pr_title="t",
