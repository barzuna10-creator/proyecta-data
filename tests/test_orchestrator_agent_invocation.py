...
class PruebaStaleSessionReused(AgentInvocationTestCase):
    def _mission_lista(self, *, provider="codex", session_id=None,
                       conversation_id="builder-thread"):
        m = _create_intake_mission("algo")
        mid = m["mission_id"]
        chugel.record_builder_evidence(mid, _builder_evidence(
            attempt=0, persisted=True, provider=provider,
    ...
    def test_mismo_proveedor_sin_identificador_comparable_falla_cerrado(self):
        ...
        result = _result(req.invocation_id, fresh_context_attested=True,
                          ...
                          provider_conversation_id=None,
                          evidence=_reviewer_evidence(attempt=0))
        with self.assertRaises(ai.IndependenceUnverifiable):
            ai.consume_emma_result(req, result)

    def test_builder_historico_sin_identidad_falla_cerrado(self):
        m = _create_intake_mission("algo")
        mid = m["mission_id"]
...
class PruebaIdentidadDeEvidencia(AgentInvocationTestCase):
    def test_emilio_inyecta_identidad_y_no_muta_evidencia_del_modelo(self):
        ...
        result = _result(
            req.invocation_id, provider="codex", provider_session_id="session-1",
            provider_conversation_id="conversation-1", evidence=evidence,
        )
        updated = ai.consume_emilio_result(req, result)
        self.assertEqual(evidence, before)
        persisted = updated["builder_evidence"][0]
        self.assertEqual(persisted["invocation_id"], req.invocation_id)
        self.assertEqual(persisted["provider"], "codex")
        self.assertEqual(persisted["provider_session_id"], "session-1")
        self.assertEqual(persisted["provider_conversation_id"], "conversation-1")

    def test_emma_inyecta_identidad_y_no_muta_evidencia_del_modelo(self):
        m = _create_intake_mission("algo")
        mid = m["mission_id"]
        chugel.record_builder_evidence(mid, _builder_evidence(attempt=0, persisted=True))
        req = ai.build_emma_invocation_request(mid, 0)
        ...
        updated = ai.consume_emma_result(req, result)
        self.assertEqual(evidence, before)
        persisted = updated["reviewer_evidence"][0]
        self.assertEqual(persisted["invocation_id"], req.invocation_id)
        self.assertEqual(persisted["provider"], "claude")
        self.assertEqual(persisted["provider_session_id"], "review-session")

    def test_modelo_no_puede_suministrar_identidad_de_infraestructura(self):
        for field in ai.INFRASTRUCTURE_EVIDENCE_FIELDS:
            with self.subTest(field=field):
                ...
                with self.assertRaises(ai.ProviderControlledIdentityMetadata):
                    ai.consume_emilio_result(req, result)
                self.assertEqual(chugel.get_mission(m["mission_id"])["builder_evidence"], [])

    def test_modelo_emma_tampoco_puede_suministrar_identidad_de_infraestructura(self):
        for field in ai.INFRASTRUCTURE_EVIDENCE_FIELDS:
            with self.subTest(field=field):
                m = _create_intake_mission(field)
                mid = m["mission_id"]
    ...
    def test_attempt_de_evidencia_debe_ser_entero_real_y_exacto(self):
    ...
    def test_attempt_de_emma_tambien_debe_ser_entero_real_y_exacto(self):
        for bad_attempt in (None, "0", False, True, -1, 1, 2):
            with self.subTest(attempt=bad_attempt):
                m = _create_intake_mission(repr(bad_attempt))
                mid = m["mission_id"]
...
class PruebaCicloCorrectivoCompleto(AgentInvocationTestCase):
    def test_secuencia_de_cuatro_invocaciones(self):
        ...
        updated = ai.consume_emma_result(req_e0, res_e0)
        ...
        updated = ai.consume_emma_result(req_e1, res_e1)
